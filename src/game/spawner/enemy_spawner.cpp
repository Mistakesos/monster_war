...
void EnemySpawner::spawn_enemy() {
    ...
    // 随机选择起点
    auto random_index = engine::utils::range_random(0, start_points.size() - 1);
    auto start_index = start_points[random_index];
    auto position = waypoint_nodes[start_index].position_;
    auto level = level_config->get_enemy_level(level_number);
    auto rarity = level_config->get_enemy_rarity(level_number);
    ...
    // 创建敌人
    entity_factory_.create_enemy_unit(enemy_type, position, start_index, level, rarity);

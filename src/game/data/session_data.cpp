...
bool SessionData::load_default_data(std::string_view path) {
...
bool SessionData::load_from_file(std::string_view path) {
    return load_default_data(path);
}

bool SessionData::save_to_file(std::string_view path) {
    std::filesystem::path file_path = path;
    // 如果父目录不存在，则自动创建
...
void SessionData::add_unit_level(entt::id_type name_id, int add_level) {
    if (auto it = unit_map_.find(name_id); it != unit_map_.end()) {
...
void SessionData::add_unit_rarity(entt::id_type name_id, int add_rarity) {
    if (auto it = unit_map_.find(name_id); it != unit_map_.end()) {

...
bool LevelConfig::load_from_file(std::string_view level_json_path) {
    ...
    try {
        ...
        for (const auto& data : json) {
            ...
            level_data.prep_time_ = sf::seconds(data["prep_time"].get<float>());
            ...
            if (data.contains("waves") && data["waves"].is_array()) {
                ...
                for (const auto& wave : data["waves"]) {
                    ...
                    wave_data.next_wave_interval_ = sf::seconds(wave["next_wave_interval"].get<float>());
                    wave_data.spawn_interval_ = sf::seconds(wave["spawn_interval"].get<float>());
                    // 每个波次中，保存了敌人类型和对应的数量

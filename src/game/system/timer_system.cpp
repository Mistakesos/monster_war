...
void TimerSystem::update(sf::Time delta) {
    update_attack_timer(delta);
    update_skill_cooldown_timer(delta);
    update_skill_duration_timer(delta);
}

void TimerSystem::update_attack_timer(sf::Time delta) {
...
void TimerSystem::update_skill_cooldown_timer(sf::Time delta) {
    ...
    for (auto&& [entity, skill] : view_skill.each()) {
        skill.cooldown_timer_ += delta;
        ...
        if (skill.cooldown_timer_ >= skill.cooldown_) {
            ...
            skill.cooldown_timer_ = sf::Time::Zero;
            // 发送技能准备就绪事件
...
void TimerSystem::update_skill_duration_timer(sf::Time delta) {
    ...
    for (auto&& [entity, skill] : view_skill.each()) {
        ...
        if (skill.duration_timer_ >= skill.duration_) {
            ...
            skill.duration_timer_ = sf::Time::Zero;
            // 发送技能持续结束事件

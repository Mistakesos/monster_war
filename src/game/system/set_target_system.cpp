...
void SetTargetSystem::update(entt::registry& registry) {
    ...
    update_no_target_player(registry);
    update_no_target_enemy(registry);
    update_healer(registry);
}

void SetTargetSystem::update_has_target(entt::registry& registry) {
    ...
    // 遍历每一个有目标的角色
    for (auto&& [entity, transform, target, stats] : view_has_target.each()) {
        // 检查目标是否还有效
        ...
        if ((transform.position_ - target_transform.position_).lengthSquared() > range_radius * range_radius) {
            // 如果在攻击范围外，则清除目标
            registry.remove<game::component::TargetComponent>(entity);
...
void SetTargetSystem::update_no_target_player(entt::registry& registry) {
    // 筛选条件：没有目标的玩家攻击型角色
    ...
    for (auto&& [player_entity, player_transform, player_stats, _] : view_player_no_target.each()) {
        ...
        for (auto&& [enemy_entity, enemy_transform, _] : view_enemy.each()) {
            ...
            if ((player_transform.position_ - enemy_transform.position_).lengthSquared() <= range_radius * range_radius) {
                // 如果敌人在攻击范围之内，则设置目标
...
void SetTargetSystem::update_no_target_enemy(entt::registry& registry) {
    ...
    // 遍历每一个没有目标的敌人角色
    for (auto&& [enemy_entity, _, enemy_transform, enemy_stats] : view_enemy_no_target.each()) {
        // 检查玩家角色是否在攻击范围之内
        for (auto&& [player_entity, player_transform, _] : view_player.each()) {
            auto range_radius = enemy_stats.range_ + game::defs::UNIT_RADIUS;
            if ((enemy_transform.position_ - player_transform.position_).lengthSquared() <= range_radius * range_radius) {
                // 如果玩家角色在攻击范围之内，则设置目标
...
void SetTargetSystem::update_healer(entt::registry& registry) {
    ...
    auto view_injured_player = registry.view<game::component::PlayerComponent, 
        ...
        engine::component::TransformComponent>();
    // 遍历每一个治疗者
    for (auto&& [healer_entity, _, healer_transform, healer_stats] : view_healer.each()) {
        // ---获取血量百分比最低的玩家角色---
        float lowest_hp_percent = 1.f;              // 保存最低血量百分比（初始为100%）
        entt::entity lowest_hp_player = entt::null; // 保存最低血量百分比的玩家角色（初始为空）
        // 遍历每一个受伤玩家角色
        for (auto&& [player_entity, _, player_stats, player_transform] : view_injured_player.each()) {
            // 如果处于治疗者攻击范围内
            auto range_radius = healer_stats.range_ + game::defs::UNIT_RADIUS;
            if ((healer_transform.position_ - player_transform.position_).lengthSquared() < range_radius * range_radius) {

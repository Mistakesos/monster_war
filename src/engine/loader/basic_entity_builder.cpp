...
void BasicEntityBuilder::reset() {
    ...
    dst_size_ = sf::Vector2f{0.f, 0.f};
    src_size_ = sf::Vector2f{0.f, 0.f};
}
...
entt::entity BasicEntityBuilder::get_entity_id() {
    return entity_id_;
}

// --- 构建组件 ---
void BasicEntityBuilder::build_base() {
    spdlog::trace("构建基础组件");
...
void BasicEntityBuilder::build_sprite() {
...
void BasicEntityBuilder::build_transform() {
    ...
    if (object_json_) {
        position_ = sf::Vector2f(object_json_->value("x", 0.f), object_json_->value("y", 0.f));
        dst_size_ = sf::Vector2f(object_json_->value("width", 0.f), object_json_->value("height", 0.f));
        position_ = sf::Vector2f(position_.x, position_.y - dst_size_.y);  // 图片对象的position需要进行调整(左下角到左上角)
...
void BasicEntityBuilder::build_audio() {
...
template<typename T>
std::optional<T> BasicEntityBuilder::get_tile_property(const nlohmann::json& tile_json, std::string_view property_name) {
    return level_loader_.get_tile_property<T>(tile_json, property_name);
}

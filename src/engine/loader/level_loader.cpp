...
void LevelLoader::set_entity_builder(std::unique_ptr<BasicEntityBuilder> builder) {
    entity_builder_ = std::move(builder);
}

bool LevelLoader::load_level(std::string_view level_path, engine::scene::Scene* scene) {
    if (!scene) {
        spdlog::error("场景指针为空");
        return false;
    }
    ...
    if (!entity_builder_) {
        ...
        entity_builder_ = std::make_unique<BasicEntityBuilder>(*this, scene->get_context(), scene->get_registry());
    }
    ...
    for (const auto& layer_json : json_data["layers"]) {
        ...
        if (layer_type == "imagelayer") {       
            load_image_layer(layer_json);
        } else if (layer_type == "tilelayer") {
            load_tile_layer(layer_json);
        } else if (layer_type == "objectgroup") {
            load_object_layer(layer_json);
        } else {
...
void LevelLoader::load_image_layer(const nlohmann::json& layer_json) {
    // 获取纹理相对路径 （会自动处理'\/\'符号）
    ...
    auto texture_path = resolve_path(image_path, map_path_); 
    auto& resource_manager = scene_->get_context().get_resource_manager();
    auto* texture = resource_manager.load_texture(entt::hashed_string(texture_path.c_str()), texture_path);
    if (!texture) {
        spdlog::error("Texture 路径 '{}' 不存在或者为空！", texture_path);
    }
    auto sprite = sf::Sprite{*texture};

    // 获取图层偏移量（json中没有则代表未设置，给默认值即可）
    const sf::Vector2f offset = sf::Vector2f(layer_json.value("offsetx", 0.0f), layer_json.value("offsety", 0.0f));
    
    // 获取视差因子及重复标志
    const sf::Vector2f scroll_factor = sf::Vector2f(layer_json.value("parallaxx", 1.0f), layer_json.value("parallaxy", 1.0f));
...
void LevelLoader::load_tile_layer(const nlohmann::json& layer_json) {
    ...
    for (const int gid : data) {
        ...
        // 使用生成器创建瓦片实体
        auto tile_entity = entity_builder_->configure(index, &tile_info.value())->build()->get_entity_id();
        // 添加到vector中
...
void LevelLoader::load_object_layer(const nlohmann::json& layer_json) {
...
sf::IntRect LevelLoader::get_texture_rect(const nlohmann::json& tileset_json, int local_id) {
    ...
    return sf::IntRect{sf::Vector2i(coordinate_x * tile_width, coordinate_y * tile_height), 
...
std::optional<engine::component::TileInfo> LevelLoader::get_tile_info_by_gid(int gid) {
    if (gid == 0) {
        return std::nullopt;
    }
    ...
    */
    ...
    if (tileset.contains("image")) {    // 这是单一图片的情况
        // 获取必要信息
        auto texture_rect = get_texture_rect(tileset, local_id);
    ...
    for (const auto& tile_json : tiles_json) {
        ...
        if (tile_id == local_id) {   // 找到对应的瓦片，进行后续操作
            ...
            if (!is_single_image) {
                ...
                // 获取图片路径
                auto texture_path = resolve_path(tile_json["image"].get<std::string>(), file_path);
                // 先确认图片尺寸
            ...
            if (tile_json.contains("animation") && is_single_image && tile_json["animation"].is_array()) {
                ...
                for (auto& frame : animation) {
                    ...
                    sf::Time duration = sf::milliseconds(frame.value("duration", 100.f));
                    int id = frame.value("tileid", 0);
                    auto frame_rect = get_texture_rect(tileset, id);  // 根据id获取纹理源矩形
                    // 源矩形 + 时长，组成一个动画帧

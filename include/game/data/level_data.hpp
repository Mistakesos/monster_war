 ...
 */
struct Wave {
    sf::Time next_wave_interval_{};    ///< @brief 下一波次间隔（单位：秒）
    sf::Time spawn_interval_{};        ///< @brief 本波次敌人生成间隔（单位：秒）
 ...
 */
struct Waves {
    sf::Time next_wave_count_down_{};   ///< @brief 下一波次倒计时（单位：秒）
...
struct LevelData {
    ...
    sf::Time prep_time_{sf::seconds(5.f)};  ///< @brief 开局准备时间（单位：秒）

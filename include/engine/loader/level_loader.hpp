#pragma once
#include "basic_entity_builder.hpp"
...
public:
    LevelLoader(engine::core::Context* context)
        : context_obs_{context} {}
    ~LevelLoader();

    /// @brief 设置实体生成器（如果不设置，则使用默认的BasicEntityBuilder）
    void set_entity_builder(std::unique_ptr<BasicEntityBuilder> builder);
     ...
     */
    [[nodiscard]] bool load_level(std::string_view level_path, engine::scene::Scene* scene);
...
private:
    ...
    void load_tile_layer(const nlohmann::json& layer_json);     ///< @brief 加载瓦片图层
    void load_object_layer(const nlohmann::json& layer_json);   ///< @brief 加载对象图层
     ...
     * @return 纹理矩形
     */
    sf::IntRect get_texture_rect(const nlohmann::json& tileset_json, int local_id);
     ...
     */
    std::optional<engine::component::TileInfo> get_tile_info_by_gid(int gid);
